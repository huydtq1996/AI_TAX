...
class TaxScheduleService:
    ...
    def get_tax_deadline_info(self, declaration_type: str, transactions: list = None, tax_payments: list = None, business_category: str = None, tax_rates: dict = None, user_token: str = None) -> dict:
        import datetime
        today = datetime.date.today()
        current_year = today.year
        current_month = today.month

        # Parse cycle from declaration_type (e.g. "quy_doanh_thu" -> "quy")
        ...
        if transactions is not None:
            ...
            for tx in transactions:
                if tx.get("date") and tx["date"].startswith(str(current_year)):
                    try:
                        amount = float(tx.get("amount") or 0)
                        if amount > 0:
                            total_annual_revenue_now += amount
                    except ValueError:
                        pass
            
            if cycle in ('quy', 'thang'):
                if total_annual_revenue_now > 50000000000:
                    if cycle == 'quy':
                        cycle = 'thang'
            ...
            declaration_type = f"{cycle}_{method}"

        # Nếu có đủ thông tin giao dịch, nộp thuế... thì tính toán động dựa trên kỳ thực tế
        if transactions is not None and tax_payments is not None and business_category is not None and tax_rates is not None:
            periods = self.get_tax_schedule_periods(
                transactions, 
                declaration_type, 
                business_category, 
                tax_payments, 
            ...
            # Tìm các kỳ chưa nộp xong có phát sinh thuế lớn hơn 0
            unpaid_periods = [p for p in periods if p.get("status") in ('unpaid', 'partial') and p.get("estimatedTax", 0) > 0]
            
            target_period = None
            if unpaid_periods:
                # Sắp xếp theo hạn nộp tăng dần (kỳ cũ nhất/gần hạn nhất lên trước)
                unpaid_periods.sort(key=lambda x: x.get("dueDate", ""))
                target_period = unpaid_periods[0]
            elif periods:
                # Nếu không có kỳ nào chưa nộp có thuế, lấy kỳ mới nhất (chạy/ongoing)
                periods.sort(key=lambda x: x.get("dueDate", ""), reverse=True)
                target_period = periods[0]
                
            if target_period:
                due_date_str = target_period["dueDate"]
                due_date = datetime.datetime.strptime(due_date_str, "%Y-%m-%d").date()
                diff_days = (due_date - today).days
                period_label = target_period["periodLabel"]
                
                if diff_days < 0:
                    message = f"Đã quá hạn nộp thuế {period_label}! Quá hạn {abs(diff_days)} ngày."
                else:
                    message = f"Sắp đến thời hạn nộp thuế! Hạn chót nộp thuế {period_label} còn {diff_days} ngày nữa."
        ...
        # Fallback về logic mặc định cũ nếu thiếu thông tin hoặc không có kỳ nào
        if cycle == 'quy':
            target_q = 1
            ...
            if current_month <= 3:
                target_q = 1
                due_date_str = f"{current_year}-04-30"
            elif current_month <= 6:
                target_q = 2
                due_date_str = f"{current_year}-07-31"
            elif current_month <= 9:
                target_q = 3
                due_date_str = f"{current_year}-10-31"
            else:
                target_q = 4
                due_date_str = f"{current_year + 1}-01-31"

            due_date_adjusted_str = self.get_adjusted_due_date(due_date_str)
            due_date = datetime.datetime.strptime(due_date_adjusted_str, "%Y-%m-%d").date()

            diff_days = (due_date - today).days

            if diff_days < 0:
                message = f"Đã quá hạn nộp thuế Quý {target_q}/{target_year}! Quá hạn {abs(diff_days)} ngày."
            else:
                message = f"Sắp đến thời hạn nộp thuế! Hạn chót nộp thuế Quý {target_q}/{target_year} còn {diff_days} ngày nữa."

            return {
                "periodLabel": f"Quý {target_q}/{target_year}",
                "dueDate": due_date_adjusted_str,
                "days": diff_days,
                "message": message,
                "autoSwitchedToMonthly": auto_switched_to_monthly,
                "autoSwitchedToQuarterly": auto_switched_to_quarterly
            ...
            due_date_str = f"{current_year}-{str(current_month).zfill(2)}-20"
            due_date_adjusted_str = self.get_adjusted_due_date(due_date_str)
            due_date = datetime.datetime.strptime(due_date_adjusted_str, "%Y-%m-%d").date()

            diff_days = (due_date - today).days
            month_label = f"Tháng {str(target_month).zfill(2)}/{target_year}"

            if diff_days < 0:
                message = f"Đã quá hạn nộp thuế {month_label}! Quá hạn {abs(diff_days)} ngày."
            else:
                message = f"Sắp đến thời hạn nộp thuế! Hạn chót nộp thuế {month_label} còn {diff_days} ngày nữa."
            ...
            return {
                ...
                "days": 10,
                "message": "Sắp đến thời hạn nộp thuế! Hạn chót nộp thuế theo từng lần phát sinh là 10 ngày.",
                "autoSwitchedToMonthly": auto_switched_to_monthly,
                "autoSwitchedToQuarterly": auto_switched_to_quarterly
            }
